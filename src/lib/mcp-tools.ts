...
export class MCPTools {
  ...
  static getProfitability(
    ...
    const grossSales = relevantSales.reduce(
      (sum, s) =>
        sum +
        (Number(s.subtotal) ||
          Number(s.total_amount) - Number(s.tax_amount || 0) ||
          0),
      0
    );
    const cogs = relevantSales.reduce(
      (sum, s) => sum + (Number(s.cogs_amount) || 0),
      0
    );
    ...
    const operatingExpenses = relevantExpenses.reduce(
    ...
    for (const exp of relevantExpenses) {
      const cat = exp.category_name || exp.title || 'General Expense';
      breakdownMap[cat] = (breakdownMap[cat] || 0) + (Number(exp.amount) || 0);
    }

    const expenseBreakdown = Object.entries(breakdownMap).map(
      ([category, amount]) => ({
        category,
        amount,
      })
  ...
  static generateBusinessReport(
    ...
    const totalRev = orgSales.reduce((acc, s) => acc + s.subtotal, 0);
    ...
    const store1Balances = LedgerService.calculateStockBalances(
      params.organization_id,
      'store_01',
      context.ledgerEntries,
      context.products
    );
    const lowStock = store1Balances.filter((b) => b.is_low_stock);
    ...
    return {
      ...
      strategic_ai_recommendations: [
        `Initiate immediate branch transfer for ${lowStock.length} critically low SKU${lowStock.length === 1 ? '' : 's'} (Alphonso Mango Pulp & Filter Coffee) from Bandra Express to Indiranagar Flagship.`,
        `Execute a flash clearance or supplier return for ${formatCurrency(deadCapital)} tied up in stagnant Saffron Tonic cases (>60 days).`,
        `Prioritize payment authorization of ${formatCurrency(apLiability)} to APMC & Amul vendors to preserve 30-day net credit lines and prevent shipment holds.`,
        `Expand shelf stock allocation for high-velocity staple categories (Basmati Rice & Sharbati Atta) yielding 38% gross margin.`
      ],
    };
  }
}
