  ...
  const fetchLiveCustomers = async () => {
    try {
      ...
      if (custData && custData.length > 0) {
        state.customers = custData as any;
        setState({ ...state });
      }
    } catch {}
  };
        ...
        {orgPromotions.map((promo) => (
          ...
          >
            <div>
              ...
              <p className="text-[11px] text-slate-300 mt-0.5">Valid until: {promo.valid_until}</p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden p-4 space-y-3">
          <div className="font-bold text-xs text-slate-900 flex items-center justify-between">
            ...
            <span className="text-[10px] text-slate-600 font-bold">{orgCustomers.length} registered</span>
          </div>
          ...
          <div className="space-y-2">
            {orgCustomers.map((cust) => (
              <button
                ...
                aria-label={`Select loyalty member ${cust.name}, Points: ${cust.loyalty_points}`}
                className={`w-full text-left p-3 rounded-xl border text-xs cursor-pointer transition flex items-center justify-between focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:outline-none ${
              ...
              >
                <div>
                  ...
                  <div className="text-[10px] text-slate-600">{cust.email}</div>
                </div>
                <div className="text-right">
