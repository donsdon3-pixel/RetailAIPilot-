...
export default function AutomationsDashboard() {
  ...
  const handleRunWorkflow = (idx: 1 | 2 | 3 | 4 | 5) => {
    setRunningIndex(idx);
    setActiveWorkflowResult(null);

    setTimeout(() => {
      const result = AppStore.runAutomation(idx);
      setActiveWorkflowResult(result);
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
          ...
          return (
            ...
            >
              <div>
                ...
                <p className="text-xs text-slate-500 mt-2 leading-relaxed">{wf.action}</p>
              </div>

              <button
                onClick={() => handleRunWorkflow(wf.id)}
                disabled={isRunning}
                className="w-full py-2.5 bg-slate-900 group-hover:bg-emerald-600 text-white rounded-xl text-xs font-bold transition flex items-center justify-center gap-1.5 shadow-sm disabled:opacity-50"
              >
                {isRunning ? (
                  <>
      ...
      {activeWorkflowResult && (
        <div className="bg-slate-900 text-white p-5 rounded-2xl border border-slate-800 shadow-xl space-y-3 font-mono text-xs animate-fadeIn">
          ...
          <div>
            <div className="text-[10px] text-slate-400 uppercase mb-1">Generated Output Payload:</div>
