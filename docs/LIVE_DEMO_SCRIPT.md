...
1. **Navigate to `/ai-assistant`:**
   ...
   - Expand the **MCP Tool Execution Card** showing the live call to `get_low_stock_products(store_id: 'store_01')` with raw JSON and dynamic database response.
...
2. **5-Tool MCP Server Suite Verification (`/api/mcp`):**
   - Demonstrate all 5 deterministic business intelligence tools:
     1. `get_low_stock_products(store_id, threshold_days)`
     2. `get_dead_stock(organization_id, min_days)`
     3. `get_profitability(store_id, start_date, end_date)` (Shows ₹13,53,880.00 Gross Sales & 27.92% Gross Margin)
     4. `get_supplier_outstanding(organization_id, min_due)` (Accounts payable schedules)
     5. `generate_business_report(organization_id, period_month)` (Full executive dossier)
3. **5 Mandatory Automated Workflows (`/automations`):**
   - Click "Trigger Workflow Now" for **Workflow 5: Monthly Executive AI Report** or **Workflow 1: Low Stock Alert**.
   - Show the generated structured executive diagnostic payload and verify that all system alert actions permanently route to `https://retail-ai-pilot.vercel.app/login`.
4. **50 QA Test Suite Verification (`/qa`):**
   - Click "Run Full 50-Test Suite Now" on `/qa` or query `GET /api/qa/run`.
   - Show 100% green pass rate across Functional (25), API (10), Security (5), AI (5), and UI (5) with real asynchronous execution timings.
